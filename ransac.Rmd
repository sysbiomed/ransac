...
if (ncol(xdata) > 100) {
  ...
  flog.info('xdata has more than 50 co-variates, showing a random subset of 50.')
} else {
...
# save(results.ransac, file = 'ransacCache.RData')
...
# save(results.ransac, results.ransac.pert, file = 'ransacCache.RData')
print.ransac.results(results.ransac.pert, xdata, ydata.pert, family = 'binomial', 'With perturbation')
```

## Test run with n = 1000

```{r set_seed_and_parameters_2}
# parameters
k              <- 1000
n              <- 30
threshold      <- .2^2
...
set.seed(my.seed)
#
results.ransac.2 <- ransac(xdata, ydata, n = n, threshold = threshold, 
                         good.fit.perct = good.fit.perct, 
                         k = k, family = 'binomial', mc.cores = mc.cores)
# save(results.ransac, results.ransac.pert, results.ransac.2, file = 'ransacCache.RData')
print.ransac.results(results.ransac.2, xdata, ydata, family = 'binomial', 'Original data')
```

### Ransac with perturbation in first 10 and last 10

```{r test_perturbation_2}
set.seed(my.seed)
#
ydata.pert <- ydata
len <- length(ydata)
...
ydata.pert[sample(which(ydata == 0),10)] <- 1
#
results.ransac.pert.2 <- ransac(xdata, ydata.pert, n = n, threshold = threshold, 
                              good.fit.perct = good.fit.perct, k = k,
                              family = 'binomial', mc.cores = mc.cores)
# save(results.ransac, results.ransac.pert, results.ransac.2, results.ransac.pert.2, file = 'ransacCache.RData')
print.ransac.results(results.ransac.pert.2, xdata, ydata.pert, family = 'binomial', 'With perturbation')
```
